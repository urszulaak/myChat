...
void writeToServer(char *to, char *content, int fserver_write, char *message, char **argv, char *downloadPath){
    ...
    if (downloadPath != NULL) {
        sprintf(message, "%s:%s:%s:%s", argv[2], to, content, downloadPath);
    } else {
...
int login(int argc, char **argv, char *downloadPath){
    ...
    if (user == -1){
        ...
        while(1){
            ...
            if ((ch = getch()) == 's') {
                writeToServer(to, content, fserver_write, message, argv, downloadPath);
            }
